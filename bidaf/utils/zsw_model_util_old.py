...
def conv1d(in_, filter_size, height, padding, is_train=None, keep_prob=1.0, scope=None):
    with tf.variable_scope(scope or "conv1d"):
        ...
        xxc = tf.nn.conv1d(in_, filter_, strides, padding) + bias  # [N, JX, W/filter_stride, d]
...
def attention_layer(logit_func, wd , is_train, h, u, h_mask=None, u_mask=None, scope=None, tensor_dict=None):
    with tf.variable_scope(scope or "attention_layer"):
        u_a, h_a = bi_attention(logit_func,wd, is_train, h, u, h_mask=h_mask, u_mask=u_mask, tensor_dict=tensor_dict)
...
def bi_attention(logit_func,wd, is_train, h, u, h_mask=None, u_mask=None, scope=None, tensor_dict=None):
    with tf.variable_scope(scope or "bi_attention"):
        ...
        u_logits = get_logits([h_aug, u_aug], None, True, wd=wd, mask=hu_mask,
                              is_train=is_train, func=logit_func, scope='u_logits')  # [N, M, JX, JQ]
